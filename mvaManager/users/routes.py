...
from mvaManager import db, bcrypt
from mvaManager.models import User, Post
from mvaManager.users.forms import registrationForm, loginForm, updateAccountForm, requestResetForm, resetPasswordForm
from mvaManager.users.utils import save_picture, send_reset_email 
...
def login():
    ...
    if form.validate_on_submit():
      ...
      if user and bcrypt.check_password_hash(user.password, form.password.data):
        ...
        return redirect(next_page) if next_page else redirect(url_for('main.home')) #ternary conditional in python
      else:
...
def reset_token(token):
  ...
  if form.validate_on_submit():
    ...
    db.session.commit()
    flash('You password has been updated! You are now able to login.', 'success')
    return redirect(url_for('users.login'))
